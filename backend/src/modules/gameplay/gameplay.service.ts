...
export class GameplayService {
  constructor(
    ...
    @Inject(forwardRef(() => GameplayGateway))
  ...
  async startGame(sessionId: string, requestingPlayerId: string): Promise<void> {
    ...
    this.gameplayGateway.emitToSession(sessionId, 'game:countdown_started', {
  ...
  async submitAnswer(
    ...
    if (existing) {
      return { accepted: false, scoreDelta: 0, error: 'ANSWER_ALREADY_SUBMITTED' };
    }
  ...
  private async openNextRound(sessionId: string): Promise<void> {
    ...
    this.gameplayGateway.emitToSession(sessionId, 'question:started', {
      roundIndex: session.currentRoundIndex,
    ...
    setTimeout(() => {
      void this.closeRound(sessionId, round.id);
    }, ROUND_DURATION_MS);
  }
  ...
  private async closeRound(sessionId: string, roundId: string): Promise<void> {
    ...
    const answerMap = new Map(round.answers.map((a) => [a.playerId, a.scoreDelta]));
    const sortedPlayers = [...players].sort((a, b) => b.score - a.score);

    this.gameplayGateway.emitToSession(sessionId, 'round:result', {
  ...
  private async finishGame(session: Session): Promise<void> {
    ...
    this.gameplayGateway.emitToSession(session.id, 'game:result', {
